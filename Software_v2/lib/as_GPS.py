...
class AS_GPS(object):
    ...
    async def _update(self, line):
        ...
        self.segx = seg0[2:]  # e.g. GLL
        if seg0.startswith('G') and self.segx in self.supported_sentences:
            try:
                s_type = self.supported_sentences[self.segx](segs)  # Parse
            except ValueError:
                s_type = False
            await asyncio.sleep(0)
            if ((self.segx == 'GGA') or (self.segx == 'GLL') or (self.segx == 'RMC') or (self.segx == 'VTG')):
...
# sentence types have been received.
    ...
    def _gprmc(self, gps_segments):  # Parse RMC sentence
        ...
        if gps_segments[8] == '':
            self.sentence_type = 'ROV'
        else:
            self.sentence_type = self.segx
    ...
    def _gpvtg(self, gps_segments):  # Parse VTG sentence
        ...
        if gps_segments[1] == '':
            self.sentence_type = 'VNC'
        else:
            self.sentence_type = 'VTG'
    ...
    def latitude(self, coord_format=DD):
        ...
        if coord_format == DD:
            ...
            seconds = (self._latitude[1] - mins) * 60
